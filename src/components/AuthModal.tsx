    ...
    try {
      ...
      if (error) {
        if (error.message.includes('provider is not enabled')) {
          toast.error('Google sign-in is not configured yet. Please use email or continue as guest.');
        } else {
      ...
      toast.error('Google sign-in is not available. Please use email or continue as guest.');
    } finally {
      setLoading(false);
    }
  };
      ...
      <DialogContent className="sm:max-w-md bg-black border border-white/20 text-white">
        ...
        <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as 'signin' | 'signup')}>
          ...
          <div className="space-y-4 mt-6">
            ...
            {/* Add a divider after the Google button */}
            <div className="relative">
              <div className="absolute inset-0 flex items-center">
                <span className="w-full border-t border-white/20" />
              </div>
              <div className="relative flex justify-center text-xs uppercase">
                <span className="bg-black px-2 text-white/60">Or use email</span>
              </div>
            </div>
